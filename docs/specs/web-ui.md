...
| GET | `/api/bootstrap` | `version`, `updateUrl`, channels (`brief` = first CONTEXT.md line, ≤80 chars), bots (`harness` / `harnessModel` / `effort`), DMs, posted counts, allowed models, `providers`, `providerCards`, `defaultPermissionMode`, `autoCompact`, `reviewerModel`, `cwd` |
...
- Chat: folds for thinking/tools, search, files chip, export, Stop, @ / palettes, composer + File / + Folder (saved under `inbox/`). Files modal: each path is a `<details>` row; `apply_patch` rows show the unified hunk in `<pre class="diff">` (no Accept/Reject). Browser screenshots from `browser_screenshot` appear in the tools fold (`GET /api/shot`, `.crew/browser/shots/*.png` only) — not in the channel account (`ADR-0050`). While a turn runs the thread shows a **working chip** — pulsing dot plus a live label following the latest status row — until the first account, tool card, or error. Live `say` folds the PNG onto the same tool row after `onToolDone` (`ADR-0054`).
- Composer: empty dock is `+` (bottom left) and **one action button** (bottom right). `+` opens File / Folder. Enter sends (IME-safe); Shift+Enter newline. `@path` from disk. `/` command palette (UI only).
  While a turn runs the action button becomes red **Stop**; typing during a run turns it into **Queue** — Enter parks the draft as a queued chip (× removes) that auto-sends oldest-first when the run finishes.
  `POST /api/say` streams `{"type":"ping"}` heartbeats every 5s and the server sets `idleTimeout: 255`, so Bun's 10s idle timeout cannot cut a silent tool phase; the UI ignores ping rows and maps a dropped stream to "Connection to the office was lost…".
- Person sheet has a **Reasoning effort** select (Default / Minimal / Low / Medium / High / Max) persisted via `PATCH /api/bot/:id` (`effort`). Default sends nothing; OpenRouter turns send `reasoning_effort`, OpenCode turns send `--variant`.
- Person/channel **Id** is a locked field. Person **Rooms** are editable only on create; after that, membership is the channel sheet.
- Desk: Discord-style `Members — N` plus a **canvas-rendered isometric floor** (`ADR-0063`, qualifies `0056`–`0060`). `floor-iso.js` is pure layout/pathing (iso projection, desk/table/door slots, BFS around obstacles; bun-tested). `floor-game.js` draws the room: checker carpet, walls + window, doors with `#signs`, glass meeting table, desks with monitors (lit when busy), procedural pixel people from `looks.json` (4-frame walk, sit/type poses, shadows, name tags, lead ★), and **typed speech bubbles**: a bot's live account types out above its head; a status dot colors the pose (thinking/working/writing). Click carpet walks You (BFS path), click a person opens `human__<id>`, click a door enters that channel, kit hold + click places furniture (owner click removes). Drag pans, wheel zooms. Data contracts unchanged: `floor.json` stays CSS-px (converted /8 to tiles), `looks.json`, presence and activity feed untouched. The scene stays alive when WebView2 throttles or pauses rAF (occluded window): a 32ms watchdog drives the loop (`0.12.9`).
...
- Channel **Files** are workspace-relative folder hints. The office strips the project cwd, resolves a unique `/api/paths` match when you pick a file or folder, and drops `..` / `.env` / `.ssh`. PATCH `/api/channel/:id` sanitizes the same way.
...
- **Empty workspace (no channels)**: panes read **No rooms yet** and boot opens the **Welcome to Crew** dialog — first channel, lead, optional teammate, work mode — one submit creates the bots then the channel (`POST /api/bots`, then `POST /api/channels` with `memberBotIds` + `leadBotId`). Skip for now hides it for the session (`sessionStorage crew.onboard.skip`). A five-step **intro tour** (office, mentions, the desk, DMs, safety rails) opens after the first setup and replays from Settings → About (`localStorage crew.tour.done`).
- Settings tabs (T3-shaped): **General** (default/fallback, new-room mode, auto-compact, People invites: Create invite / list / revoke; token once, hash never), **Providers** (OpenRouter key/base URL/whitelist + Claude/Codex/Grok/OpenCode cards with custom models; replaces Models), **Jobs**, **MCP** (stdio or HTTP servers in `.crew/mcp.json`; tools, resources, and prompts on Crew-native turns), **Permissions** (Always add/remove + reviewer), **About** (version + workspace path; Updates: GitHub Releases feed on by default with a checkbox, custom URL override, Check button, **Install <version>**). Updates flow per `ADR-0062`. Crew.exe × hides to tray (`ADR-0039`); update feed + signed silent install per `ADR-0062`.
